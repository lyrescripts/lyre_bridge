...
function bridge:hasPermission(groups)
	...
	for groupName, grade in pairs(self.object:GetGroups() or {}) do
		playerGroups[groupName] = grade
	end

	local groupsType = type(groups)

	if groupsType == "string" then
		if playerGroups[groups] ~= nil or self.object:HasGroup(groups) then
			return true
		end
	...
	elseif groupsType == "table" then
		...
		if isArray then
			local permsToCheck = {}
			for i = 1, #groups do
				if playerGroups[groups[i]] ~= nil or self.object:HasGroup(groups[i]) then
					return true
				end
				-- Check ACE permissions from cache
