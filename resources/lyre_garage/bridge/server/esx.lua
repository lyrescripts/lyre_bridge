...
local function getPlayerSourcesByIdentifier(esxObject)
	...
	local values = {}
	local players = esxObject.GetExtendedPlayers()

	for _, xPlayer in pairs(players) do
		if xPlayer.identifier and xPlayer.source then
			values[xPlayer.identifier] = xPlayer.source
		end
	end
...
function bridge:isVehicleOwned(plate)
	...
	local result = MySQL.scalar.await("SELECT 1 FROM `owned_vehicles` WHERE plate = ? LIMIT 1", { normalizedPlate })
...
function bridge:retrieveVehicles(type, owner)
	local typeFilter = ""
	if type == "job" then
		typeFilter = " AND ov.`lyre_garage-job_vehicle` = 1"
	elseif type == "gang" then
		typeFilter = " AND ov.`lyre_garage-gang_vehicle` = 1"
	elseif type == "player" then
		typeFilter = " AND (ov.`lyre_garage-job_vehicle` IS NULL OR ov.`lyre_garage-job_vehicle` != 1) AND (ov.`lyre_garage-gang_vehicle` IS NULL OR ov.`lyre_garage-gang_vehicle` != 1)"
	end
	...
	local selectColumns = [[
		...
		ov.plate,
		ov.vehicle,
		ov.`lyre_garage-location`,
		ov.`lyre_garage-location_name`,
		ov.`lyre_garage-job_vehicle`,
		ov.`lyre_garage-gang_vehicle`,
		ov.`lyre_garage-job_rank`,
		ov.`lyre_garage-gang_rank`,
		ov.`lyre_garage-impound_fee`,
		ov.`lyre_garage-allow_owner_retrieve`,
		ov.`lyre_garage-impound_min_retrieve_date`,
		ov.`lyre_garage-impound_reason`,
		ov.`lyre_garage-impounded_by`,
		ov.`lyre_garage-impounded_at`,
		ov.`lyre_garage-fuel`,
		ov.owner
	]]
	...
	local query = string.format(
		...
		INNER JOIN `owned_vehicles` ov ON ov.plate = sa.plate
		LEFT JOIN `users` u ON ov.owner = u.identifier
		WHERE sa.shared_with_identifier = ? AND ov.owner != ?%s
	]],
		selectColumns,
		typeFilter,
		selectColumns,
	...
	local response = MySQL.query.await(query, { owner, owner, owner })
	...
	if response then
		...
		for i = 1, #response do
			...
			-- Handle NULL values and convert to boolean
			...
			-- type == "all" allows all vehicles regardless of job/gang status
			...
			if canPass then
				...
				if not isOwner then
					...
					local ownerPlayer = self:getPlayerFromIdentifier(row.owner)
					if ownerPlayer then
						vehicleData.ownerName = ownerPlayer.getName()
...
function bridge:checkVehicleOwnership(plate, owner)
	local normalizedPlate = trim(plate)
	local result = MySQL.scalar.await("SELECT 1 FROM `owned_vehicles` WHERE plate = ? AND owner = ? LIMIT 1", { normalizedPlate, owner })
...
function bridge:getVehicleOwner(plate)
	local normalizedPlate = trim(plate)
	local result = MySQL.scalar.await("SELECT owner FROM `owned_vehicles` WHERE plate = ?", { normalizedPlate })
	return result
end
...
function bridge:saveVehicleProperties(plate, vehicleProperties)
	...
	MySQL.update(query, { json.encode(vehicleProperties), normalizedPlate }, function(affectedRows)
...
function bridge:getVehicleName(plate)
	local normalizedPlate = trim(plate)
	local result = MySQL.scalar.await("SELECT `lyre_garage-name` FROM `owned_vehicles` WHERE plate = ?", { normalizedPlate })
	return result
end
...
function bridge:saveVehicleName(plate, name)
	local normalizedPlate = trim(plate)
	-- Fire-and-forget: no need to wait for completion
	MySQL.update("UPDATE `owned_vehicles` SET `lyre_garage-name` = ? WHERE plate = ?", { name, normalizedPlate })
end
...
function bridge:saveVehicleLocation(plate, location, locationName, callback)
	...
	MySQL.update("UPDATE `owned_vehicles` SET `lyre_garage-location` = ?, `lyre_garage-location_name` = ? WHERE plate = ?", { location, locationName, normalizedPlate }, callback)
end
...
function bridge:clearVehicleLocation(plate)
	local normalizedPlate = trim(plate)
	-- Fire-and-forget: no need to wait for completion
	MySQL.update("UPDATE `owned_vehicles` SET `lyre_garage-location` = NULL, `lyre_garage-location_name` = NULL WHERE plate = ?", { normalizedPlate })
end
...
function bridge:saveVehicleFuel(plate, fuel)
	local normalizedPlate = trim(plate)
	-- Fire-and-forget: no need to wait for completion
	MySQL.update("UPDATE `owned_vehicles` SET `lyre_garage-fuel` = ? WHERE plate = ?", { fuel, normalizedPlate })
end
...
function bridge:getVehicleLocationName(plate)
	local normalizedPlate = trim(plate)
	local result = MySQL.scalar.await("SELECT `lyre_garage-location_name` FROM `owned_vehicles` WHERE plate = ?", { normalizedPlate })
	return result
end
...
function bridge:getVehicleJobRank(plate)
	local normalizedPlate = trim(plate)
	local result = MySQL.scalar.await("SELECT `lyre_garage-job_rank` FROM `owned_vehicles` WHERE plate = ?", { normalizedPlate })
	return result or 0
end
...
function bridge:getVehicleGangRank(plate)
	local normalizedPlate = trim(plate)
	local result = MySQL.scalar.await("SELECT `lyre_garage-gang_rank` FROM `owned_vehicles` WHERE plate = ?", { normalizedPlate })
	return result or 0
end
...
function bridge:countPlayerVehiclesInGarage(owner, garageId, excludePlate, callback)
	local query = [[
		...
		FROM `owned_vehicles`
		WHERE `lyre_garage-location_name` = ? AND owner = ? AND plate != ?
	]]

	MySQL.query(query, { garageId, owner, excludePlate or "" }, function(result)
...
function bridge:giveVehicle(owner, vehicleModel, plate, isJobVehicle, isGangVehicle, callback)
	...
	local insertQuery = [[
		INSERT INTO `owned_vehicles` (owner, plate, vehicle, `lyre_garage-job_vehicle`, `lyre_garage-gang_vehicle`, `lyre_garage-fuel`)
		VALUES (?, ?, ?, ?, ?, 100.00)
	]]

	MySQL.insert(insertQuery, { owner, plate, vehicleProperties, isJobVehicle and 1 or 0, isGangVehicle and 1 or 0 }, function(insertId)
		if insertId then
...
function bridge:checkPlateExists(plate, callback)
	local normalizedPlate = trim(plate)
	local query = "SELECT 1 as existing FROM `owned_vehicles` WHERE plate = ? LIMIT 1"
...
function bridge:deleteVehicleFromFramework(plate, callback)
	...
	-- Delete from owned_vehicles (framework table)
	local query = "DELETE FROM `owned_vehicles` WHERE plate = ?"
	MySQL.query(query, { normalizedPlate }, function(result)
		if result then
...
function bridge:changePlate(oldPlate, newPlate, callback)
	...
	local checkQuery = "SELECT 1 as existing FROM `owned_vehicles` WHERE plate = ? LIMIT 1"
...
function bridge:retrieveAllImpoundedVehicles(impoundId)
	local query = [[
		SELECT
			ov.plate,
			ov.vehicle,
			ov.`lyre_garage-name` as vehicleName,
			ov.`lyre_garage-location` as location,
			ov.`lyre_garage-location_name` as locationId,
			ov.`lyre_garage-impound_fee` as impoundFee,
			ov.`lyre_garage-allow_owner_retrieve` as allowOwnerRetrieve,
			ov.`lyre_garage-impound_min_retrieve_date` as impoundMinRetrieveDate,
			ov.`lyre_garage-impound_reason` as impoundReason,
			ov.`lyre_garage-impounded_by` as impoundedBy,
			ov.`lyre_garage-job_vehicle` as isJobVehicle,
			ov.`lyre_garage-gang_vehicle` as isGangVehicle,
			ov.owner as ownerIdentifier,
			CONCAT(u.firstname, ' ', u.lastname) as ownerName
		FROM `owned_vehicles` ov
		LEFT JOIN `users` u ON ov.owner = u.identifier
		WHERE ov.`lyre_garage-location` = 'impound'
	]]

	local params = {}

	if Config.uniqueImpounds and impoundId then
		query = query .. " AND ov.`lyre_garage-location_name` = ?"
	...
	if response then
		for i = 1, #response do
			...
			table.insert(vehicles, {
				...
				ownerName = row.ownerName or "Unknown",
...
function bridge:getSharedAccessList(plate)
	...
	local response = MySQL.query.await(
		[[
		SELECT sa.shared_with_identifier, u.firstname, u.lastname
		FROM `lyre_garage-shared_access` sa
		LEFT JOIN `users` u ON sa.shared_with_identifier = u.identifier
		WHERE sa.plate = ?
		]],
		{ normalizedPlate }
	)
...
function bridge:retrievePlayerImpoundedVehicles(playerIdentifier, jobName, gangName)
	local query = [[
		SELECT
			...
			ov.`lyre_garage-job_vehicle` as isJobVehicle,
			ov.`lyre_garage-gang_vehicle` as isGangVehicle,
			ov.`lyre_garage-fuel` as fuel
		FROM `owned_vehicles` ov
		WHERE ov.`lyre_garage-location` = 'impound'
		AND (
			(ov.owner = ? AND ov.`lyre_garage-job_vehicle` = 0 AND ov.`lyre_garage-gang_vehicle` = 0)
	]]

	local params = { playerIdentifier }

	if jobName then
		query = query .. " OR (ov.owner = ? AND ov.`lyre_garage-job_vehicle` = 1)"
		table.insert(params, jobName)
	end

	if gangName then
		query = query .. " OR (ov.owner = ? AND ov.`lyre_garage-gang_vehicle` = 1)"
