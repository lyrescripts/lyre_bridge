local provider = LyreBridge.registerProvider("server", "vehicle_storage", "qbcore", 20)
...
function provider:getProperties(plate)
    plate = plate and plate:gsub("^%s*(.-)%s*$", "%1")
    local raw = MySQL.scalar.await("SELECT mods FROM `player_vehicles` WHERE plate = ?", { plate })
...
function provider:getInfo(plate)
    plate = plate and plate:gsub("^%s*(.-)%s*$", "%1")
    local row = MySQL.single.await("SELECT plate, citizenid, mods FROM `player_vehicles` WHERE plate = ?", { plate })
